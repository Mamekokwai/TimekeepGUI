...
- 正式构建与安装包仍使用默认生产配置。开发入口的保护不得改变 `tauri build`、发布签名或安装版升级路径。

这条边界的目的不是禁止共存，而是保证共存时迁移、设置、备份、单实例锁与 WebView 状态互不污染。数据库 migration 只负责同一运行身份内的版本升级，不能承担开发版与安装版之间的并发兼容。
