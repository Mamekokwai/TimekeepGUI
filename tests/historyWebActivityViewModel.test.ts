...
runTest("web timeline merges same-domain rows and folds matching titles within threshold", () => {
  ...
  assert.deepEqual(items[0].titleSamples, ["Mamekokwai/TimekeepGUI"]);
...
runTest("web timeline treats domain like app and keeps page titles as details", () => {
  const items = buildWebTimelineItems([
    makeSegment({
      ...
      domain: "github.com",
      normalizedDomain: "github.com",
      title: "Issues · TimekeepGUI",
      url: "https://github.com/Mamekokwai/TimekeepGUI",
      startTime: 0,
      endTime: 5_000,
    }),
    makeSegment({
      ...
      domain: "github.com",
      normalizedDomain: "github.com",
      title: "Pull requests · TimekeepGUI",
      url: "https://github.com/Mamekokwai/TimekeepGUI",
      startTime: 7_000,
      endTime: 10_000,
    }),
  ...
  assert.equal(items[0].mergedCount, 2);
  assert.deepEqual(items[0].titleSamples, ["Issues · TimekeepGUI", "Pull requests · TimekeepGUI"]);
  assert.deepEqual(items[0].titleSampleDetails, [
    {
      title: "Issues · TimekeepGUI",
      secondaryText: "https://github.com/Mamekokwai/TimekeepGUI",
      startTime: 0,
      endTime: 5_000,
      duration: 5_000,
    },
